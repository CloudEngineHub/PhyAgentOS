...
**Minecraft workflow** (no separate HAL process needed):

The session watchdog runs inside the Agent process (started automatically by `paos agent`). Use `write_file` or `edit_file` to append sessions to `SESSIONS.md` instead of `execute_robot_action`. See `RUNTIME.md` for the SESSIONS.md format.

**Manipulation sim workflow** (after watchdog is running):

1. **`enter_simulation`** (or **`start`**) with **`parameters: {}`** when the watchdog was started with `--driver-config …/pipergo2_manipulation_driver.json`. The driver already holds `scene_asset_path` and objects; you do **not** need to pass them again.
